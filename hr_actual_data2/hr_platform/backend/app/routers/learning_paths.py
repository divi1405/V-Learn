...
def list_paths(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    paths = db.query(LearningPath).options(
        joinedload(LearningPath.courses).joinedload(PathCourse.course)
    ).all()
    
    def score_path(p):
        s = 0
        if p.department and user.department and p.department.lower() == user.department.lower():
            s -= 10
        if p.target_role and user.designation and p.target_role.lower() in user.designation.lower():
            s -= 10
        elif p.target_role and user.role and p.target_role.lower() in user.role.lower():
            s -= 5
        return s
