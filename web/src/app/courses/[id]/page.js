                            ...
                            <div>
                                ...
                                {activeLesson.type === 'quiz' ? (
                                    <div className="quiz-container">
                                        {!quiz ? (
                                            ...
                                            <div className={`quiz-result ${quizResult.passed ? 'passed' : 'failed'}`}>
                                                ...
                                                {!quizResult.passed && (
                                                    <button className="btn btn-secondary" style={{ marginRight: 12 }} onClick={() => { setQuizResult(null); setQuizAnswers({}); }}>Reattempt Quiz</button>
                                                )}
                                                {quizResult.passed && nextLesson ? (
                                                    <button className="btn vearc-btn-primary" onClick={() => handleSelectLesson(nextLesson)}>Next Lesson →</button>
                                                ) : quizResult.passed ? (
                                                    <button className="btn vearc-btn-primary" onClick={() => { setQuizResult(null); setQuizAnswers({}); setQuiz(null); }}>Finish</button>
                                                ) : null}
                                                {/* Always allow dismissing the quiz results pane */}
                                                {!quizResult.passed && (
                                                    <button className="btn btn-ghost" onClick={() => { setQuizResult(null); setQuizAnswers({}); setQuiz(null); }}>Close</button>
                                                )}
                                            </div>
                                        ) : (
                                            <div>
                                    ...
                                    <div>
                                        {/* YouTube Video Embed */}
                                        {videoId ? (
                                            <div className="video-container" key={`yt-${activeLesson.id}`}>
                                                <iframe
                                            ...
                                            <div className="video-container" key={`vid-${activeLesson.id}`}>
                                                <video
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        if (parent && !parent.querySelector('.video-error')) {
                                                            ...
                                                            msg.style.cssText = 'padding:40px;text-align:center;color:var(--vearc-text-muted);background:var(--vearc-surface-soft);border-radius:var(--vearc-radius-md);border:1px solid var(--vearc-border)';
                                                            msg.innerHTML = '<div style="font-size:2rem;margin-bottom:8px">🎬</div><p style="margin:0;font-weight:600">Video unavailable</p><p style="margin:4px 0 0;font-size:0.85rem">This video file could not be loaded. It may have been removed or is temporarily unavailable.</p>';
