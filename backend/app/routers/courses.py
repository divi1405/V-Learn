...
from app.models import User, CourseReview
...
def get_course_enrollees(
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """Return enrolled users per course as { course_id: [{id, name}, ...] }."""
    enrollments = (
        db.query(Enrollment)
    ...
    for e in enrollments:
        ...
        result[cid].append({
            ...
            "profile_image": getattr(e.user, "profile_image", None) if e.user else None
        })
    return result
