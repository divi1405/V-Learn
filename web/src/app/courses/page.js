...
export default function CoursesPage() {
    ...
    // My Courses: all enrolled (self or assigned), in progress
    const myCoursesList = filteredCourses.filter(c => {
        const e = enrollments.find(env => env.course_id === c.id);
        return e && e.status !== 'completed';
    });

    // Completed Courses
    const completedCoursesList = filteredCourses.filter(c => {
        const e = enrollments.find(env => env.course_id === c.id);
        return e && e.status === 'completed';
    });

    // All other courses (not enrolled)
    ...
    const renderCourseCard = (c) => {
        ...
        const isAssigned = enrollment && enrollment.assigned_by;
        return (
            <div key={c.id} className="card" style={{ cursor: 'pointer', overflow: 'hidden', padding: '24px' }} onClick={() => router.push(`/courses/${c.id}`)}>
                ...
                {/* Rating stars — click to view all reviews */}
                {review && (
                    <div
                        style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12, cursor: 'pointer' }}
                        title="View all reviews for this course"
                        onClick={e => { e.stopPropagation(); router.push(`/feedback?course=${c.id}`); }}
                    >
                        <div className="stars">
                            {[1, 2, 3, 4, 5].map(s => (
                                <span key={s} className={`star ${s <= Math.round(parseFloat(review.avg)) ? 'filled' : ''}`} style={{ cursor: 'pointer', fontSize: '0.85rem' }}>★</span>
                            ))}
                        </div>
                        ...
                        <span style={{ fontSize: '0.7rem', color: 'var(--vearc-primary)', marginLeft: 2 }}>→ View</span>
                    </div>
                )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: '0.8rem', color: 'var(--vearc-text-muted)' }}>
                    ...
                    <span><BookIcon width="14" height="14" /> {c.modules?.length || 0} {(c.modules?.length || 0) === 1 ? 'module' : 'modules'}</span>
            ...
            <main className="main-content" style={{ padding: 0 }}>
                ...
                <div style={{ padding: '24px' }}>
                    ...
                    <div className="vearc-tabs">
                        ...
                        <div className={`vearc-tab ${activeTab === 'mycourses' ? 'active' : ''}`} onClick={() => setActiveTab('mycourses')}>My Courses</div>
                        <div className={`vearc-tab ${activeTab === 'completed' ? 'active' : ''}`} onClick={() => setActiveTab('completed')}>Completed Courses</div>
                        <div className={`vearc-tab ${activeTab === 'toprated' ? 'active' : ''}`} onClick={() => setActiveTab('toprated')}>Top Rated</div>
                    </div>

                    <div className="course-grid">
                        {activeTab === 'all' && (
                            allCoursesList.length > 0 ? allCoursesList.map(renderCourseCard) : (
                        ...
                        {activeTab === 'mycourses' && (
                            myCoursesList.length > 0 ? myCoursesList.map(renderCourseCard) : <div style={{ gridColumn: '1 / -1', color: 'var(--vearc-text-muted)', textAlign: 'center', padding: '40px 0' }}>No courses in progress. Enroll in a course to get started!</div>
                        )}
                        {activeTab === 'completed' && (
                            completedCoursesList.length > 0 ? completedCoursesList.map(renderCourseCard) : <div style={{ gridColumn: '1 / -1', color: 'var(--vearc-text-muted)', textAlign: 'center', padding: '40px 0' }}>No completed courses yet. Keep learning!</div>
                        )}
                        {activeTab === 'toprated' && (
