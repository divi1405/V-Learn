                ...
                <div style={{
                    ...
                    {enrollment && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{Math.round(enrollment.progress_pct)}%</span>
                        </div>
                    )}
                    {!enrollment && !isAdmin && <button className="btn btn-primary btn-sm" onClick={handleEnroll}>Enroll Now</button>}
                </div>
                ...
                <div className="player-layout" style={{ flex: 1 }}>
                    ...
                    <div className="player-sidebar">
                        {course.modules?.map((mod) => (
                            <div key={mod.id}>
                                <div style={{ padding: '12px 16px', fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>{mod.title}</div>
                                {mod.lessons?.map((lesson) => (
                                    ...
                                    >
                                        <span className={`check ${isLessonCompleted(lesson.id) ? 'done' : ''}`}>
                                            {isLessonCompleted(lesson.id) ? '✓' : lesson.type === 'quiz' ? '📝' : lesson.type === 'video' ? '▶' : '📄'}
                                        </span>
                                        <span style={{ flex: 1 }}>{lesson.title}</span>
                                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{lesson.duration_mins}m</span>
                                    </button>
                                ))}
                            </div>
                        ))}
                        ...
                        {activeLesson ? (
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                    ...
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>⏱ {activeLesson.duration_mins} min</span>
                                </div>
                                ...
                                {activeLesson.type === 'quiz' ? (
                                    <div className="quiz-container">
                                        {!quiz ? (
                                            <div style={{ textAlign: 'center', padding: 40 }}>
                                                ...
                                                <p style={{ color: 'var(--text-secondary)', marginBottom: 24 }}>Test your knowledge from this section</p>
                                            ...
                                            <div className={`quiz-result ${quizResult.passed ? 'passed' : 'failed'}`}>
                                                ...
                                                {!quizResult.passed && (
                                                    <button className="btn btn-primary" onClick={() => { setQuizResult(null); setQuizAnswers({}); }}>Retry</button>
                                                )}
                                                {quizResult.passed && nextLesson && (
                                                    <button className="btn btn-primary" onClick={() => handleSelectLesson(nextLesson)}>Next →</button>
                                                )}
                                            </div>
                                        ) : (
                                            <div>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'space-between' }}>
                                            ...
                                            {nextLesson && (
                                                <button className="btn btn-secondary" style={{ marginLeft: 'auto' }} onClick={() => handleSelectLesson(nextLesson)}>
