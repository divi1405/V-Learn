    ...
    return (
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <svg width="100" height="100" viewBox="0 0 100 100">
                ...
                <text x="50" y="60" textAnchor="middle" fontSize="10" fill="var(--text-muted)">/ {max}</text>
            </svg>
...
export default function ProfilePage() {
    ...
    const streak = Math.floor(Math.random() * 12) + 1; // mock streak

    // Rank tiers based on completion count
    const getRank = (n) => {
        if (n >= 20) return { label: 'Expert Learner', icon: '🏆', color: '#ffd700' };
        if (n >= 10) return { label: 'Advanced', icon: '💎', color: '#b39ddb' };
        if (n >= 5) return { label: 'Intermediate', icon: '🚀', color: '#42a5f5' };
        if (n >= 1) return { label: 'Beginner', icon: '🌱', color: '#66bb6a' };
        return { label: 'New Joiner', icon: '⭐', color: 'var(--text-muted)' };
    };
        ...
        return (
            <div style={{ marginTop: 4 }}>
                <div style={{ display: 'flex', gap: 3 }}>
                    {[1, 2, 3, 4].map(i => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= s ? colors[s] : 'var(--border)' }} />)}
                </div>
            ...
            <main className="main-content">
                ...
                <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 24, flexWrap: 'wrap' }}>
                    <div style={{
                        ...
                        justifyContent: 'center', fontSize: '2rem', fontWeight: 700, color: '#fff', flexShrink: 0
                    }}>{user.name?.[0]}</div>
                    <div style={{ flex: 1 }}>
                        <h2 style={{ margin: '0 0 4px', fontSize: '1.4rem' }}>{user.name}</h2>
                        <div style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginBottom: 6 }}>
                            {user.designation || 'Employee'} · {user.division || user.department || 'VeARC'}
                        </div>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            ...
                            <span style={{ fontSize: '0.8rem', color: rank.color, fontWeight: 700 }}>{rank.label}</span>
                        </div>
                    </div>
                    <button className="btn btn-ghost btn-sm" onClick={() => setShowPwForm(!showPwForm)}>
                        🔑 Change Password
                    </button>
                </div>
                ...
                <div className="card" style={{ marginBottom: 24 }}>
                    <h3 style={{ marginTop: 0, marginBottom: 16 }}>👤 My Details</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                        {[
                            ...
                            <div key={label} style={{ padding: '10px 14px', background: 'var(--bg)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                ...
                {showPwForm && (
                    <div className="card" style={{ marginBottom: 24, border: '1px solid var(--accent)', boxShadow: '0 0 0 2px rgba(var(--accent-rgb),0.1)' }}>
                        <h3 style={{ marginTop: 0, marginBottom: 4 }}>🔐 Change Password</h3>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 20 }}>
                            Enter your current password, then set a new secure one.
                        </p>
                        {pwError && <div style={{ background: 'rgba(229,57,53,0.1)', border: '1px solid rgba(229,57,53,0.4)', color: '#ff6b6b', padding: '10px 14px', borderRadius: 'var(--radius)', fontSize: '0.85rem', marginBottom: 12 }}>{pwError}</div>}
                        {pwSuccess && <div style={{ background: 'rgba(0,200,83,0.1)', border: '1px solid rgba(0,200,83,0.4)', color: '#00c853', padding: '10px 14px', borderRadius: 'var(--radius)', fontSize: '0.85rem', marginBottom: 12 }}>{pwSuccess}</div>}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                ...
                                <div className="form-group">
                                    ...
                                    {pwForm.confirm && pwForm.new !== pwForm.confirm && (
                                        <div style={{ fontSize: '0.72rem', color: '#e53935', marginTop: 4 }}>Doesn't match</div>
                                    )}
                                </div>
                            </div>
                ...
                {/* Progress Stats — LeetCode/Codeforces style */}
                ...
                <div className="card" style={{ marginBottom: 24 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: 24, padding: '8px 0' }}>
                        <StatRing value={completed} max={total} color="#00c853" label="Completed" sublabel="courses finished" />
                        <StatRing value={inProgress} max={total} color="#42a5f5" label="In Progress" sublabel="currently studying" />
                        <StatRing value={badges.length} max={20} color="#ffd700" label="Badges" sublabel="earned" />
                        <StatRing value={certs.length} max={total} color="#b39ddb" label="Certificates" sublabel="awarded" />
                    </div>
                </div>
                ...
                <div className="stats-grid" style={{ marginBottom: 24 }}>
                    ...
                    <div className="stat-card">
                        <div className="stat-value" style={{ fontSize: '1.2rem' }}>{rank.icon} {rank.label}</div>
                ...
                {/* Course Breakdown */}
                <div className="card" style={{ marginBottom: 24 }}>
                    <h3 style={{ marginTop: 0, marginBottom: 16 }}>📚 Course Breakdown</h3>
                    {[
                        { label: 'Completed', count: completed, color: '#00c853' },
                        { label: 'In Progress', count: inProgress, color: '#42a5f5' },
                ...
                {enrollments.length > 0 && (
                    <div>
                        <div className="section-header"><h2>📋 All My Courses</h2></div>
                                ...
                                <div key={e.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 20px', cursor: 'pointer' }}
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                                            <div style={{ flex: 1, height: 4, background: 'var(--border)', borderRadius: 2, maxWidth: 200 }}>
                                                <div style={{ height: '100%', width: `${e.progress_pct || 0}%`, background: e.status === 'completed' ? '#00c853' : 'var(--gradient)', borderRadius: 2 }} />
                                            </div>
                                            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{Math.round(e.progress_pct || 0)}%</span>
                                        </div>
                                    </div>
