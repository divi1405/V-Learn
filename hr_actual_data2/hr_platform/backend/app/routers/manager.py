...
from app.models import User, Enrollment, EnrollmentStatus, Course, LessonProgress, Notification, UserRole
...
def get_portco_employees(
    ...
    if manager.role in (UserRole.ADMIN, UserRole.HR_ADMIN):
        ...
        employees = db.query(User).filter(User.manager_id == manager.id).all()
